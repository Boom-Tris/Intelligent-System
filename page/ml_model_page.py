...
def display_ml_model():
    ...
    st.markdown('<div class="highlight">การสร้าง Dataset โดยใช้ GPT</div>', unsafe_allow_html=True)
    ...
    try:
        ...
        # แสดงข้อมูลในไฟล์
        st.markdown('<div class="highlight">ข้อมูลในไฟล์ที่อัปโหลด:</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="text_indent">การเติมค่าที่หายไปด้วยค่าเฉลี่ย (Mean Imputation)การเติมค่าที่หายไปด้วยค่าเฉลี่ยของแต่ละคอลัมน์ (เช่น ค่าเฉลี่ยของราคาปิด) เป็นวิธีที่ง่ายและนิยมใช้กันมากที่สุด เนื่องจากช่วยให้ข้อมูลสมบูรณ์โดยไม่ต้องตัดข้อมูลออกจากการพิจารณา </div>', unsafe_allow_html=True)
        st.markdown('<div class="text_indent">การใช้ค่าที่ใกล้เคียง (Forward Fill หรือ Backward Fill)บางครั้งค่าที่ขาดหายไปอาจเกิดขึ้นเนื่องจากการเก็บข้อมูลไม่สมบูรณ์ในบางช่วงเวลา ในกรณีนี้การใช้ค่าที่มีอยู่แล้วในแถวก่อนหน้าหรือหลัง (เช่น ใช้ค่าก่อนหน้าหรือหลังเติมค่าที่หายไป) อาจเป็นวิธีที่ช่วยรักษาความต่อเนื่องของข้อมูลได้ </div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="big-font">วิธีการเตรียมข้อมูลเบื้องต้น</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="highlight">1.ตรวจสอบคอลัมน์ที่จำเป็น</div>', unsafe_allow_html=True)
        code = '''
...
X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, test_size=0.2, random_state=42
        ...
        st.markdown('<div class="normal-text">• วิธีการทำงาน: KNN ใช้แนวคิดที่ว่า "สิ่งที่คล้ายกันจะอยู่ใกล้กัน" ในการทำนายราคาหุ้นโดยการหาความสัมพันธ์ระหว่างข้อมูลการซื้อขายในอดีตและผลลัพธ์ที่ต้องการ โดยดูจากข้อมูลที่ใกล้เคียงที่สุด (neighbors) กับข้อมูลใหม่ที่ต้องการทำนาย <br>'
                    '• การใช้ในโปรเจคนี้: KNN จะทำการทำนายราคาปิดของหุ้นโดยอ้างอิงข้อมูลจากราคาปิดในช่วงเวลาใกล้เคียงและใช้ค่าเฉลี่ยจากเพื่อนบ้านที่มีลักษณะคล้ายกัน เช่น ราคาสูงสุด, ราคาต่ำสุด, หรือค่าเฉลี่ยเคลื่อนที่<br></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="normal-text">• วิธีการทำงาน: Decision Tree สร้างโมเดลที่เป็นโครงสร้างต้นไม้เพื่อการตัดสินใจ โดยการแบ่งข้อมูลออกเป็นกลุ่มๆ ตามคุณสมบัติต่างๆ เช่น ราคาสูงสุด, ราคาต่ำสุด, หรือการเปลี่ยนแปลงของราคาหุ้น <br>'
                    '• การใช้ในโปรเจคนี้: โมเดลนี้จะใช้ในการแบ่งข้อมูลทางการเงินและการทำนายราคาหุ้นตามลักษณะต่างๆ เช่น การใช้ค่าเฉลี่ยเคลื่อนที่ 50 และ 200 วัน เพื่อให้สามารถตัดสินใจได้ว่าแนวโน้มราคาหุ้นจะเป็นอย่างไรในอนาคต<br></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="normal-text">• วิธีการทำงาน: SVR เป็นการนำ Support Vector Machine (SVM) มาประยุกต์ใช้ในการทำนายค่าตัวเลข (regression) โดยพยายามหาฟังก์ชันที่ดีที่สุดในการทำนายราคาหุ้นจากข้อมูลทางการเงิน <br>'
                    '• การใช้ในโปรเจคนี้: SVR จะใช้ข้อมูลเชิงลึกของราคาหุ้นและค่าต่างๆ เช่น ราคาสูงสุดและราคาต่ำสุด เพื่อคาดการณ์ราคาปิดในอนาคต โดยคำนึงถึงความสัมพันธ์ระหว่างตัวแปรต่างๆ และไม่พยายามจับคู่ข้อมูลเกินไป (ไม่เกิด overfitting)<br></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="normal-text">• วิธีการทำงาน: Ensemble Method รวมหลายโมเดลเข้าด้วยกันเพื่อเพิ่มความแม่นยำในการทำนาย โดยการใช้โมเดลต่างๆ เช่น KNN, Decision Tree, และ SVR มารวมกันเพื่อให้ผลลัพธ์ที่มีความแม่นยำสูงสุด<br>'
                    '• การใช้ในโปรเจคนี้: ในการทำนายราคาหุ้น SET สามารถนำโมเดลหลายตัวมารวมกันและใช้เทคนิคการ stacking เพื่อให้ผลลัพธ์การทำนายมีความเสถียรและแม่นยำมากขึ้น<br></div>', unsafe_allow_html=True)
